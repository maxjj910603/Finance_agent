...
class LLMClient:
    ...
    def _cleanup_answer(self, text: str) -> str:
        cleaned = text.strip()
        ...
        cleaned = re.sub(r"\n{3,}", "\n\n", cleaned).strip()
        return self._normalize_traditional_chinese(cleaned)

    def _normalize_traditional_chinese(self, text: str) -> str:
        normalized = text
        for source, target in self._S2T_PHRASES:
            normalized = normalized.replace(source, target)
        normalized = normalized.translate(self._S2T_CHARS)
        return normalized
